...
If you want to run hyperboria on a system where you are not able to access a TUN device,
...
If you create the socket before hyperboria starts up, it will connect to it, otherwise it
...
which the hyperboria node wants to configure. After receiving this message, you know which
source address you should use when crafting a packet to send to the network.
NOTE: hyperboria will drop packets with an unexpected source address.

## SocketWrapper_TYPE_CONF_SET_MTU
This type of packet starts with 1 byte `2` and is followed by a 4 byte MTU, hyperboria
