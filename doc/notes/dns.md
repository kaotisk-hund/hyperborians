# Random notes about hyperboria and DNS
...
## DNS on hyperboria/parabolic/any hyperboria network
...
- rainflyDNS (ironically, by hyperboria) # built off of namecoin? integrated into hyperboria. development (seemingly) halted # correct me if I'm wrong.
  - https://pad.meshwith.me/p/rainflydns #thx
  - http://gitboria.com/hyperboria/rainflydns/
  - http://rfdns.hyperboria.ca/
...
- .h # nice and short
  - confusing when we consider that hyperboria is heavily related to C
...
Would it be possible to have it be a part of hyperboria? eg one less step to use? // it kind of is, RAINFLYDNS, but the servers aren't running yet. We have 4 people running them, the system requires the cooperation of 13. If 8 others want to volunteer, I am willing to be the fifth. hyperboria gave his 'seal of approval'.
...
it was proposed (by prurigro) that an address be somehow based upon the owner's hyperboria IPV6
...
To what extent should it be linked to hyperboria?

- having a dns module be an inherent part of hyperboria means it should (in theory) be faster than alternatives.
...
- The problem is not how we do DNS, it's how we know who to trust. Once we know that a node can be trusted, we can trust any traffic coming from it to be authentic, thanks to hyperboria
  - Thus, we don't need to reinvent the wheel when it comes to DNS protocols, we just need to make hyperboria smart enough to find trustworthy DNS servers
...
- All normal hyperboria nodes should be able to communicate and share a list of active DNS servers to query
...
- A person can register a third level domain for an active hyperboria address (eg jack.sandiego.mesh)
...
- A hyperboria node should attempt to resolve a random sampling of domains for every name server that it knows about (obviously only .sandiego.mesh domains for a sandiego.mesh nameserver)
...
<derp> the real beauty of hyperboria is it solves network neutrality
...
<ansuz> but like, such that no one is in charge, I think that's what hyperboria was getting at with the nmc thing
...
<derp> ansuz, hyperboria's nmc thing is way to hopeful. Its like he read Richard Stallmans dreams, but yeah, we need something *realistically better than the status quo*
...
<qmx> ansuz: I like hyperboria's idea of using .h as root
...
<prurigro> kinda like how the ipv6 is cryptographically generated a much longer, but single source in hyperboria
<prurigro> (the pubkey)
<prurigro> and a word + 7 characters is infinitely more remembreable than an ipv6
<qmx> so I could technically have blog.fcea93
<ansuz> don't you then have to change your domain if you change your hyperboria conf?
<ansuz> change/lose
...
<qmx> since hyperboria is built upon "know thy peers"
...
<qmx> which runs almost "inside" hyperboria

<jackv> but the key is that you need to trust someone, once you have that, hyperboria makes it possible to trust all the traffic coming from him as his own
...
```
<larsg> jercos: do you have any spontaneous thought re: web-of-trust-based dns on top of hyperboria peers?
...
<hyperboria> I've thought about some ideas like that
...
<hyperboria> I killed a few ideas off
<hyperboria> what if we just make a rule that we'll pull any changes to the db as long as they match the requirements and come from someone you trust
<hyperboria> requirements: can't hijack a domain which is already regged
<hyperboria> that's arguably a tragedy of the commons though since everyone will race to create new domains as fast as those connected to them will allow
<larsg> that description is a lot more concise, thnx
<hyperboria> in this context, trust is binary
<hyperboria> I add your node to the list -> I trust you absolutely
<hyperboria> I'll pull anything you offer
<hyperboria> like bgp
<larsg> or just for .com domains, or just for names matching /f.*ck/
<hyperboria> what's wrong with  firetruck.hype ?
<larsg> :]
<hyperboria> and why not match against flappingcuntlips ?
<hyperboria> or  throw-the-jew-down-the-well.hype
<hyperboria> if you're going to censor, you have a duty to do it right ;)
<larsg> de-peer
<larsg> err, untrust
<hyperboria> hmm
<hyperboria> the way I imagined it, there's no clear way to drop a domain once it's registered
<hyperboria> except for blocking it from being regged again and waiting for it to expire
<larsg> yeah but "registered" is relative to whom you trust
<hyperboria> meh that won't work
<hyperboria> you have to reach consensus
<larsg> y?
<hyperboria> b/c all (working) dns does
<hyperboria> if you rely on trust to determine what is the truth, you're going to have problems
...
<hyperboria> ok so you're actually thinking of doing the whole internet
<larsg> cacert for dns i guess, with different tech underneath
<larsg> useless without the community around it
<larsg> just like hyperboria
<hyperboria> well you could write some sort of a rules engine for defining the domains
<hyperboria> keep in mind though that it goes against how domains are normally registered
<hyperboria> err
<hyperboria> looked up
<larsg> most communities and individuals will want to resolve icann domains the icann way
<hyperboria> well you could write a rule like   .com -> 4.2.2.1
<hyperboria> .net -> 8.8.8.8
<hyperboria> penis.com -> 123.45.67.8   (if you reject how icann is handling it)
<hyperboria> or you could just say . -> fc00:.....friend
<hyperboria> hmm
<hyperboria> when a node spins up, if it has no dns config it could just pull from it's nearest peer
<larsg> or it'd "ask what do to do"
<hyperboria> asking is a sin
<larsg> yep indeed
<hyperboria> just search for the nearest node which is offering dns results
<larsg> FBI_Surveillance_Van_3
<larsg> or do you mean search in the dht?
<hyperboria> search the dht
<hyperboria> surveillence van will still fuck you, it just needs to be running hyperboria
<larsg> that'd be a sensible default i guess
<hyperboria> but that's kind of the rule
<hyperboria> the van always wins
<hyperboria> unless you do a big public key signing shitfest
...
<hyperboria> tor uses a list of 8 directory servers
<hyperboria> search the DHT for a directory server
<hyperboria> kind of rainfly method
<hyperboria> but directory servers die and stuff
<hyperboria> not so resilient
